...
async function uploadCanvasToBackend(snapshotCanvas) {
  try {
    if (!snapshotCanvas) return;
    const blob = await new Promise((resolve) =>
      snapshotCanvas.toBlob(resolve, "image/jpeg", 0.92)
    );
    if (!blob) return;
    ...
    if (!res.ok) {
      console.error("[PuzzleCam] backend upload failed:", res.status);
      return;
    }

    const data = await res.json();
    console.log("[PuzzleCam] uploaded:", data);
  } catch (err) {
    console.error("[PuzzleCam] upload error:", err);
  }
}

function uploadCanvasToBackendAsync(snapshotCanvas) {
  uploadCanvasToBackend(snapshotCanvas).catch((e) =>
    console.error("[PuzzleCam] upload async error:", e)
  );
}
...
function finishShatter() {
  ...
  if (shatter.pendingCanvas) {
    ...
    // upload saved photo to backend
    uploadCanvasToBackendAsync(shatter.pendingCanvas);
